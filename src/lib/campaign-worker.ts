...
export async function processCampaignQueue() {
  ...
  try {
    ...
    while (true) {
      ...
      if (stillRunning) {
        await sleep(computeDelay(sentIndex, profile));
        sentIndex += 1;
      }
    }
  } finally {
...
async function processJob(job: ClaimedJob): Promise<"ok" | "paused" | "wait"> {
  ...
  const picked = pickRandomInstance(pool, safeProfile(campaign.profile));
  if (!picked) {
    await prisma.campaignContact.update({
      where: { id: job.id },
      data: { status: "pending", claimedAt: null },
...
export async function resumeCampaignRun(runId: string) {
  ...
  await prisma.campaignRun.update({
    where: { id: runId },
    data: { status: "RUNNING", pauseReason: null, consecutiveFailures: 0 },
  });
  await prisma.campaign.update({
    where: { id: run.campaignId },
    data: { status: "RUNNING" },
